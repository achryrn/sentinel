...
|---|---|---|
| File events | ✅ ReadDirectoryChangesW (per-directory, event-driven, no polling) | Curated roots: Startup folders, Downloads, Temp, ProgramData\Sentinel, service dirs |
| Process creation | ✅ WMI `__InstanceCreationEvent` on Win32_Process | 2–10 s latency class, elevated for full fidelity; snapshot reconciliation every 30–60 s as backstop |
| Persistence (registry) | ✅ RegNotifyChangeKeyValue on curated keys | Per-key handles; documented |
| Persistence (tasks/services) | ⚠️ Periodic reconciliation (bounded, e.g. 5 min) | No supported push API; documented |
| Network | ✅ Bounded polling (2 s) + event linkage | No driver; documented |
| Behavior | ✅ Evidence stream + correlation engine | Same pipeline as scans |
